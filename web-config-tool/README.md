...
- ⚡ **ESP32 网页烧录**：集成 Web Serial API，支持浏览器直接烧录 ESP32-C3
...
- **浏览器兼容性**：需要 Chrome 89+ 或 Edge 89+ 浏览器
- **设备连接**：点击"连接设备"按钮，选择 ESP32 设备
- **Web Serial API**：使用现代浏览器的 Web Serial API 进行设备通信
- **完整日志**：提供详细的连接日志和状态反馈
- **备用方案**：当网页烧录不可用时，提供命令行烧录指导

> **注意**：ESP32 网页烧录功能需要 esptool-js 库支持。如果库未正确加载，工具会提供详细的命令行烧录指导。
